...
internal class RecordAclResponse : BaseToData, ISqlTable
{
    ...
    public List<RecordNotificationValue> ListRight { get; set; } = new();
    ...
    #region ISqlTable
    public static string TableName(bool withCamma_) => typeof(RecordAclResponse).TableName(withCamma_);
    ...
    {
        const int _SORT = 100;
        var rtn = typeof(RecordAclResponse).ListColumn();
        rtn.AddRange(typeof(RecordNotificationValue).ListColumn(string.Empty, _SORT));
        rtn.AddRange(typeof(RecordNotificationValueBase).ListColumn(string.Empty, _SORT+ _SORT));
        return rtn;
    }
