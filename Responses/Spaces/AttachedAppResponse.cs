...
internal class AttachedAppResponse : BaseToData
{
    //attachedApps[].threadId	文字列	スレッドID
    [JsonPropertyName("threadId")]
    [ColumnEx("threadId", Order = 1, TypeName = "TEXT", IsUnique =true)]
    public string ThreadId { get; set; } = string.Empty;

    //attachedApps[].appId	文字列	アプリID
    [JsonPropertyName("appId")]
    [ColumnEx("appId", Order = 2, TypeName = "TEXT", IsUnique = true)]
    public string AppId { get; set; } = string.Empty;

    //attachedApps[].code	文字列	アプリコード
    [JsonPropertyName("code")]
    [ColumnEx("code", Order = 3, TypeName = "TEXT")]
    public string Code { get; set; } = string.Empty;

    //attachedApps[].name	文字列	アプリの名前
    [JsonPropertyName("name")]
    [ColumnEx("name", Order = 4, TypeName = "TEXT")]
    public string Name { get; set; } = string.Empty;

    //attachedApps[].description	文字列	アプリの説明
    [JsonPropertyName("description")]
    [ColumnEx("description", Order = 5, TypeName = "TEXT")]
