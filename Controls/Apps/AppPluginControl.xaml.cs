...
/// </summary>
public partial class AppPluginControl : UserControl, INotifyPropertyChanged
{
    public AppPluginControl()
    {
        InitializeComponent();
    }
    ...
    {
        get => _appView.AppPluginDateTime;
        set
        {
            _appView.AppPluginDateTime = value;
        ...
        if (_dataGrid.ItemsSource is DataView view)
        {
            if (view.Count == 0)
            {
                ...
                win.Run = async () =>
                {
                    ...
                    var response = await AppPluginRequest.Instance.Insert(_appView.AppId);
    ...
    private async Task _loadDatabase()
    {
        ...
        _dataGrid.ItemsSource = (await SQLiteManager.Instance.SelectTable(ListAppPluginResponse.TableName(false), $"WHERE {Resource.COLUMN_APP_ID}='{_appView.AppId}'")).DefaultView;
