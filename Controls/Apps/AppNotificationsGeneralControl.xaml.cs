...
/// </summary>
public partial class AppNotificationsGeneralControl : UserControl, INotifyPropertyChanged
{
    public AppNotificationsGeneralControl()
    {
        InitializeComponent();
    }
    ...
    {
        get => _appView.NotificationsGeneralDateTime;
        set
        {
            _appView.NotificationsGeneralDateTime = value;
    ...
    private async void _getClick(object sender_, RoutedEventArgs e_)
    {
        if (_dataGrid.ItemsSource is DataView view)
        {
            if (view.Count == 0)
            {
                ...
                win.Run = async () =>
                {
                    progresssBarCount?.Invoke(0, 1, "Notifications General");
                    var response = await AppNotificationsGeneralRequest.Instance.Insert(_appView.AppId);
    ...
    private async Task _loadDatabase()
    {
        ...
        _dataGrid.ItemsSource = (await SQLiteManager.Instance.SelectTable(ListAppNotificationsGeneralResponse.TableName(false), $"WHERE {Resource.COLUMN_APP_ID}='{_appView.AppId}'")).DefaultView;
