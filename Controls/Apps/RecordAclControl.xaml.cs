...
/// <summary>
/// RecordAclControl.xaml の相互作用ロジック
/// </summary>
public partial class RecordAclControl : UserControl, INotifyPropertyChanged
{
    public RecordAclControl()
    {
        InitializeComponent();
    }
    ...
    {
        get => _appView.RecordAclDateTime;
        set
        {
            _appView.RecordAclDateTime = value;
    ...
    private async void _getClick(object sender_, RoutedEventArgs e_)
    {
        if (_dataGrid.ItemsSource is DataView view)
        {
            if (view.Count == 0)
            {
                ...
                win.Run = async () =>
                {
                    progresssBarCount?.Invoke(0, 1, "Record Acl");
                    var response = await RecordAclRequest.Instance.Insert(_appView.AppId);
    ...
    private async Task _loadDatabase()
    {
        ...
        _dataGrid.ItemsSource = (await SQLiteManager.Instance.SelectTable(ListRecordAclResponse.TableName(false), $"WHERE {Resource.COLUMN_APP_ID}='{_appView.AppId}'")).DefaultView;
