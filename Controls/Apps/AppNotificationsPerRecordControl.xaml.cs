...
/// <summary>
/// AppNotificationsPerRecordControl.xaml の相互作用ロジック
/// </summary>
public partial class AppNotificationsPerRecordControl : UserControl, INotifyPropertyChanged
{
    public AppNotificationsPerRecordControl()
    {
        InitializeComponent();
    }
    ...
    {
        get => _appView.NotificationsPerRecordDateTime;
        set
        {
            _appView.NotificationsPerRecordDateTime = value;
        ...
        if (_dataGrid.ItemsSource is DataView view)
        {
            if (view.Count == 0)
            {
                ...
                win.Run = async () =>
                {
                    ...
                    var response = await AppNotificationsPerRecordRequest.Instance.Insert(_appView.AppId);
    ...
    private async Task _loadDatabase()
    {
        ...
        _dataGrid.ItemsSource = (await SQLiteManager.Instance.SelectTable(ListAppNotificationsPerRecordResponse.TableName(false), $"WHERE {Resource.COLUMN_APP_ID}='{_appView.AppId}'")).DefaultView;
