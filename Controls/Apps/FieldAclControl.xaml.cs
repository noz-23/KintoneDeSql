...
/// <summary>
/// FieldAclControl.xaml の相互作用ロジック
/// </summary>
public partial class FieldAclControl : UserControl, INotifyPropertyChanged
{
    public FieldAclControl()
    {
        InitializeComponent();
    }
    ...
    {
        get => _appView.FieldAclDateTime;
        set
        {
            _appView.FieldAclDateTime = value;
        ...
        if (_dataGrid.ItemsSource is DataView view)
        {
            if (view.Count == 0)
            {
                ...
                win.Run = async () =>
                {
                    ...
                    var response = await FieldAclRequest.Instance.Insert(_appView.AppId);
    ...
    private async Task _loadDatabase()
    {
        ...
        _dataGrid.ItemsSource = (await SQLiteManager.Instance.SelectTable(ListFieldAclResponse.TableName(false), $"WHERE {Resource.COLUMN_APP_ID}='{_appView.AppId}'")).DefaultView;
