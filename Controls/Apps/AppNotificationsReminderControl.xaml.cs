...
/// <summary>
/// AppNotificationsReminderControl.xaml の相互作用ロジック
/// </summary>
public partial class AppNotificationsReminderControl : UserControl, INotifyPropertyChanged
{
    public AppNotificationsReminderControl()
    {
        InitializeComponent();
    }
    ...
    private async void _getClick(object sender_, RoutedEventArgs e_)
    {
        if (_dataGrid.ItemsSource is DataView view)
        {
            if (view.Count == 0)
            {
                ...
                win.Run = async () =>
                {
                    progresssBarCount?.Invoke(0, 1, "Notifications Reminder");
                    var response = await AppNotificationsReminderRequest.Instance.Insert(_appView.AppId);
    ...
    private async Task _loadDatabase()
    {
        ...
        _dataGrid.ItemsSource = (await SQLiteManager.Instance.SelectTable(ListAppNotificationsReminderResponse.TableName(false), $"WHERE {Resource.COLUMN_APP_ID}='{_appView.AppId}'")).DefaultView;
