...
/// <summary>
/// AppCustomizeControl.xaml の相互作用ロジック
/// </summary>
public partial class AppCustomizeControl : UserControl, INotifyPropertyChanged
{
    public AppCustomizeControl()
    {
        InitializeComponent();
    }
    ...
    {
        get => _appView.AppCustomizeDateTime;
        set
        {
            _appView.AppCustomizeDateTime = value;
        ...
        if (_dataGrid.ItemsSource is DataView view)
        {
            if (view.Count == 0)
            {
                ...
                win.Run = async () =>
                {
                    ...
                    var response = await AppCustomizeRequest.Instance.Insert(_appView.AppId);
    ...
    private async Task _loadDatabase()
    {
        ...
        _dataGrid.ItemsSource = (await SQLiteManager.Instance.SelectTable(AppCustomizeResponse.TableName(false), $"WHERE {Resource.COLUMN_APP_ID}='{_appView.AppId}'")).DefaultView;
