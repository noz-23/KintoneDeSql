...
/// <summary>
/// AppSettingControl.xaml の相互作用ロジック
/// </summary>
public partial class AppSettingControl : UserControl, INotifyPropertyChanged
{
    ...
    {
        get => _appView.AppSettingDateTime;
        set
        {
            _appView.AppSettingDateTime = value;
        ...
        if (_dataGrid.ItemsSource is DataView view)
        {
            if (view.Count == 0)
            {
                ...
                win.Run = async () =>
                {
                    ...
                    var response = await AppSettingRequest.Instance.Insert(_appView.AppId);
    ...
    private async Task _loadDatabase()
    {
        ...
        _dataGrid.ItemsSource = (await SQLiteManager.Instance.SelectTable(AppSettingResponse.TableName(false), $"WHERE {Resource.COLUMN_APP_ID}='{_appView.AppId}'")).DefaultView;
