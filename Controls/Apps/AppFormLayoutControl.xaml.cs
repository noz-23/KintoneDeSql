...
/// <summary>
/// AppFormLayout.xaml の相互作用ロジック
/// </summary>
public partial class AppFormLayout : UserControl, INotifyPropertyChanged
{
    ...
    {
        get => _appView.AppFormLayoutDateTime;
        set
        {
            _appView.AppFormLayoutDateTime = value;
    ...
    private async void _getClick(object sender_, RoutedEventArgs e_)
    {
        if (_dataGrid.ItemsSource is DataView view)
        {
            if (view.Count == 0)
            {
                ...
                win.Run = async () =>
                {
                    progresssBarCount?.Invoke(0, 1, "App Form Layouts");
                    var response = await AppFormLayoutsRequest.Instance.Insert(_appView.AppId);
    ...
    private async Task _loadDatabase()
    {
        ...
        _dataGrid.ItemsSource = (await SQLiteManager.Instance.SelectTable(ListAppFormLayoutResponse.TableName(false), $"WHERE {Resource.COLUMN_APP_ID}='{_appView.AppId}'")).DefaultView;
    }
}
